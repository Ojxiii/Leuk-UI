...
AddEventHandler('onResourceStart', function(resource) if GetCurrentResourceName() ~= resource then return end
	TriggerEvent("Leuk-shops:MakeStash")
...
RegisterServerEvent('Leuk-shops:ShopOpen', function(shop, name, shoptable)
	local data = { shoptable = { products = shoptable.items, label = shoptable.label, color = shoptable.color, }, custom = true }
	TriggerClientEvent('Leuk-shops:ShopMenu', source, data, true)
end)
...
RegisterServerEvent('Leuk-shops:GetItem', function(amount, billtype, item, shoptable, price, info, shop, num, nostash)
	local src = source
			...
			for i = 1, amount do -- Make a loop to put items into different slots rather than full amount in 1 slot
				if Player.Functions.AddItem(item, 1, nil, info) then
					if tonumber(i) == tonumber(amount) then -- when its on its last loop do this
						...
						TriggerClientEvent("Leuk-shops:SellAnim", src, {item = item, shoptable = shoptable})
					end
				else
		...
		if Config.Limit and not nostash then
			...
			for i = 1, #stashItems do
				if stashItems[i].name == item then
					...
					TriggerEvent('Leuk-shops:server:SaveStashItems', stashname, stashItems)
	...
	TriggerClientEvent('Leuk-shops:ShopMenu', src, data, custom)
end)

RegisterNetEvent("Leuk-shops:MakeStash", function()
	...
	for k, v in pairs(Config.Locations) do
		if k ~= "vendingmachine" then
			...
			if Config.Limit then TriggerEvent('Leuk-shops:server:SaveStashItems', "["..k.."("..l..")]", stashTable)
...
RegisterNetEvent("Leuk-shops:GenerateVend", function(data)
	local stashTable = {}
	...
	TriggerEvent('Leuk-shops:server:SaveStashItems', data[2], stashTable)
end)
...
RegisterNetEvent("qb-shops:server:RestockShopItems", function(storeinfo)
	...
	if Config.Limit then TriggerEvent('Leuk-shops:server:SaveStashItems', "["..k.."("..l..")]", stashTable) end
end)

QBCore.Functions.CreateCallback('Leuk-shops:server:getLicenseStatus', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('Leuk-shops:server:sellChips', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('Leuk-shops:server:GetStashItems', function(source, cb, stashId) cb(GetStashItems(stashId)) end)
RegisterNetEvent('Leuk-shops:server:SaveStashItems', function(stashId, items) MySQL.Async.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', { ['stash'] = stashId, ['items'] = json.encode(items) }) end)
