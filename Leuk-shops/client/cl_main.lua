		...
		if k == "vendingmachine" and Config.VendOverride then
			for l, b in pairs(v["coords"]) do
				...
				exports['qb-target']:AddTargetModel(v["model"], {
					options = { {
						event = "Leuk-shops:ShopMenu", icon = (v["targetIcon"]), label = (v["targetLabel"]),
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { { event = "Leuk-shops:ShopMenu", icon = (v["targetIcon"] or "fas fa-circle"), label = (v["targetLabel"] or "Browse"), item = v["requiredItem"], job = v["job"] or nil, gang = v["gang"] or nil,
				shoptable = v, name = v["label"], k = k, l = l, }, }
				if k == "casino" then
					options[#options+1] = { type = "server", event = "Leuk-shops:server:sellChips", icon = "fab fa-galactic-republic", label = "Trade Chips ($"..Config.SellCasinoChips.pricePer.." per chip)", }
				end
				Targets["Shop - ['"..k.."("..l..")']"] =
...
RegisterNetEvent('Leuk-shops:ShopMenu', function(data, custom)
	...
	if Config.Limit and data.vend then
		...
		local p = promise.new()
		QBCore.Functions.TriggerCallback('Leuk-shops:server:GetStashItems', function(stash) p:resolve(stash) end, vendID)
		...
		if json.encode(stashItems) == "[]" then
			...
			TriggerServerEvent("Leuk-shops:GenerateVend", {data, vendID})
			Wait(1000)
			local p = promise.new()
			QBCore.Functions.TriggerCallback('Leuk-shops:server:GetStashItems', function(stash) p:resolve(stash) end, vendID)
			stashItems = Citizen.Await(p)
	...
	if Config.Limit and not custom and not data.vend then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('Leuk-shops:server:GetStashItems', function(stash) p:resolve(stash) end, "["..data.k.."("..data.l..")]")
		stashItems = Citizen.Await(p)
	end
	...
	if data.shoptable["type"] == "weapons" then
		...
		QBCore.Functions.TriggerCallback("Leuk-shops:server:getLicenseStatus", function(hasLic, hasLicItem) p:resolve(hasLic) p2:resolve(hasLicItem) end)
	...
	for i = 1, #products do
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
			...
			for k, v in pairs(products[i].requiredJob) do
				if QBCore.Functions.GetPlayerData().job.name == k and QBCore.Functions.GetPlayerData().job.grade.level >= v then
					...
					ShopMenu[#ShopMenu + 1] = { icon = icon, header = header, txt = text, isMenuHeader = lock,
						params = { event = "Leuk-shops:Charge", args = { item = products[i].name, cost = products[i].price, info = products[i].info or {}, shoptable = data.shoptable, k = data.k, l = data.l, amount = amount, custom = custom } } }
					end
				end
			...
			elseif products[i].requiresLicense then
				...
				if products[i].info ~= nil and products[i].info.label ~= nil then header = products[i].info.label else header = setheader end
				ShopMenu[#ShopMenu + 1] = { icon = icon, header = header, txt = text, isMenuHeader = lock,
				params = { event = "Leuk-shops:Charge", args = {
	...
	SendNUIMessage({
		app = 'openShop',
		shopName = data.shoptable["label"],
		items = ShopMenu,
		shopColor = data.shoptable["color"],
	})
	-- exports['qb-menu']:openMenu(ShopMenu)
end)
--Selling animations are simply a pass item to seller animation

RegisterNetEvent('Leuk-shops:SellAnim', function(data)
...
RegisterNetEvent('Leuk-shops:CloseMenu', function() exports['qb-menu']:closeMenu() end)

RegisterNUICallback('Leuk-shops:Charge', function(data)
	...
	local newinputs = {
		{ type = 'radio', name = 'billtype', text = settext, options = { 
	...
	local dialog = exports['qb-input']:ShowInput({ header =  data.shoptable["label"] .. " | " .. QBCore.Shared.Items[data.item].label, submitText = "Pay", inputs = newinputs })
	if dialog then
		if not dialog.amount then return end
		if Config.Limit and data.custom == nil then	if tonumber(dialog.amount) > tonumber(data.amount) then TriggerEvent("QBCore:Notify", "Incorrect amount", "error") TriggerEvent("Leuk-shops:Charge", data) return end end
		if tonumber(dialog.amount) <= 0 then TriggerEvent("QBCore:Notify", "Incorrect amount", "error") TriggerEvent("Leuk-shops:Charge", data) return end
		if data.cost == "Free" then data.cost = 0 end
		if data.amount == nil then nostash = true end
		TriggerServerEvent('Leuk-shops:GetItem', dialog.amount, dialog.billtype, data.item, data.shoptable, data.cost, data.info, data.k, data.l or nil, nostash)
	end
end)
