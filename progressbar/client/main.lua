...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if IsShowProgressbar then
            local playerPed = PlayerId()
            local playerCoords = GetEntityCoords(GetPlayerPed(-1))
            local pedCoords = GetEntityCoords(GetPlayerPed(playerPed))
            local onScreen, screenX, screenY = GetHudScreenPositionFromWorldPosition(pedCoords.x, pedCoords.y, pedCoords.z + 0.1)
            print(screenX, screenY)

            if not onScreen then
                SendNUIMessage({
                    action = "Hologramprogress",
                    x = screenX * -40,
                    y = screenY * 100,
                })
            end
